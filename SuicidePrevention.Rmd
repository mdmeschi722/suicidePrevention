---
title: "Suicide Prevention - Data Science For Business Final Project"
author: "Mark D. Meschi"
date: "March 17, 2019"
...
Suicide is an international plague which impacts not only the victim, but the family and friends of the victim. 
**What if we could use data science to help predict people at risk of suicide to target mental health resources?** 

<div class = centered > 
![](C:\Users\Mark\OneDrive\Documents\idea2.jpg)
</div>
...
The data used for this work was found on the Kaggle website: <https://www.kaggle.com/russellyates88/suicide-rates-overview-1985-to-2016#master.csv>

It contains data from 1985 to 2016 with socio-economic information with suicide rates by year and country. 
The ultimate sources of the information compiled on the Kaggle website follow: 

* United Nations Development Program. 
* World Bank 
* Suicide in the Twenty-First Century (also on Kaggle)
* World Health Organization


## Architecture 

The architecture for my work on this project is relatively simple.  I don't have access to Visio, so I was unable to draw a nice picture.  However, the following was done from an architectural standpoint: 

* Retrieve .csv file from the internet
* Pull in .csv file into Rstudio 
* Pull in .csv file into Microsoft Power BI
* Extract data from Rstudio into excel for massaging, then reimport into RStudio

```{r setUpForPlots, echo=FALSE, comment=NA, include=FALSE, cache=TRUE}
require(ggplot2)
require(tidyr)
require(dplyr)
require(xts)
...
barHighPlotCountries <- ggplot(dfHighMeanSuicideByCountry, aes(x=country, y = mean )) + 
    geom_bar(aes(fill = country), stat = "identity") + theme_bw() + labs(title = "Ten Countries with the Highest Suicide Rates") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
barHighPlotCountries

barLowPlotCountries <- ggplot(dfLowMeanSuicideByCountry, aes(x=country, y = mean)) + 
    geom_bar(aes(fill = country), stat = "identity") + theme_bw() + labs(title = "Ten Countries with the lowest Suicide Rates") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
...
barGeo <- ggplot(dfMeanSuicideRateByGeographicRegion, aes(x=GeographicRegion, y = Mean )) + 
    geom_bar(aes(fill = GeographicRegion), stat = "identity") + theme_bw() + labs(title = "Suicide Rate By Region") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
barGeo
...
cm <- confusionMatrix(test_pred, testing$suicideBy100KCat2)
...
cm_gini <- confusionMatrix(test_pred_gini, testing$suicideBy100KCat2)
...
Use Microsoft Power BI to get a sense of the impact of Countries and age on the Suicide rates. 

<div class = "centered">
![](C:\Users\Mark\Documents\MapSize2.png)

</div>

## Data Analysis 

```{r densityPlot, comment="", echo=FALSE}

plotDensity

```

###### Min = 0 | 1st Qu. = .92 | Median = 5.99 | Mean = 12.82, 3rd Qu. = 16.62 | Max = 224.97

##Data Analysis 

```{r plot1, comment="", echo=FALSE}
linePlotSuicidesByYear
...
```{r plot2, comment="", echo=FALSE}
  linePlotReportingCountries
```

## Data Analysis 

```{r plot3, comment="", echo=FALSE}
linePlotMeanSuicidesPer100kByYear
```

## Data Analysis 

```{r plot4, comment="", echo=FALSE}
barHighPlotCountries
...
```{r plot6, comment="", echo=FALSE}
barGeo

```

## Data Analysis 
```{r plot7, comment="", echo=FALSE}
boxplot(dfMasterRecords$suicidesPer100k ~ dfMasterRecords$sex, 
        main = "Suicides Per 100k by Sex")
...
<div class = "centered">
![](C:\Users\Mark\Documents\DataDict.png)
</div>

## Decision Tree 

Use decision tree with cross fold validation with 70% - 30% split between training and testing data 
...
## Decision Tree Classifier With IG Criterion 
```{r plot21, comment="", echo=FALSE}
...
## D-Tree Classifier With IG Criterion 

<div class = "centered">
![](C:\Users\Mark\Documents\IGTree.png)
</div>

## Decision Tree Classifier with Gini Criterion 
```{r plot22, comment="", echo=FALSE}
...
## D-Tree Classifier With Gini Criterion
<div class = "centered">
![](C:\Users\Mark\Documents\DtreeGini.jpg)

</div>
## Thank You 

* Code can be found on github: https://github.com/mdmeschi72/suicidePrevention
* Questions? 
